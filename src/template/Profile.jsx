  ...
  const handleFilter = async () => {
    ...
    try {
      const response = await axios.post(
        'http://192.168.1.15:8080/api/videos/filter',
        filterData,
      );
      const filteredVideos = response.data;

      console.log('Filtered videos received:', filteredVideos); // Log the filtered videos

      // Check if the response is correct before setting state
      if (Array.isArray(filteredVideos) && filteredVideos.length > 0) {
        const videosWithUri = filteredVideos.map(video => ({
          ...video,
          uri: `http://192.168.1.15:8080/api/videos/user/${video.id}`,
        }));
        ...
        setFilteredVideos([]); // Handle no videos returned
      }
    } catch (err) {
